  ...
  case "$os_raw" in
    Linux)
      if has ldd && ldd /bin/sh 2>&1 | grep -qi musl; then
        die "Alpine/musl Linux is not supported by the current release assets. Add linux-musl-$arch to the release to support it."
      fi

      case "$arch" in
        x64)
          if [ -e /lib/ld-musl-x86_64.so.1 ]; then
            die "Alpine/musl Linux is not supported by the current release assets. Add linux-musl-x64 to the release to support it."
          fi
          ;;
        arm64)
          if [ -e /lib/ld-musl-aarch64.so.1 ]; then
            die "Alpine/musl Linux is not supported by the current release assets. Add linux-musl-arm64 to the release to support it."
          fi
          ;;
      esac
